<template>
  <div v-wechat-title="$route.meta.title" class="container">
    <div class="item">
      <van-field v-model="username" label="收货人：" placeholder="请输入收货人" />
    </div>
    <div class="item">
      <van-field v-model="username" label="手机号码：" placeholder="请输入手机号码" />
    </div>
    <div class="item" @click="show=true">
      <van-field v-model="username" label="所在地区：" placeholder="请选择省市区" />
    </div>
    <div class="item">
      <van-field v-model="username" label="详细地址：" placeholder="如街道、门牌号、小区、楼栋号、单元室" />
    </div>
    <div class="item">
      <div class="label">
        设为默认：
        <div class="switch upDown">
          <van-switch v-model="checked" size="20px" />
        </div>
      </div>
    </div>
    <van-popup v-model="show" position="bottom" >
      <van-area :area-list="areaList" @confirm="chooseArea"/>
    </van-popup>

    <div class="sub">确定</div>
  </div>
</template>

<script>
import area from "../../../utils/area";
export default {
  data() {
    return {
      username: "",
      code: "",
      new: "",
      areaList: area,
      show: true,
      checked: false
    };
  },
  methods: {
    chooseArea(e){
      console.log(e)
    },
    init() {}
  },

  mounted() {
    this.init();
  }
};
</script>
<style scoped lang="scss">
@import "./style/index.scss";
</style>
<style scoped>
>>> .van-popup {
  background: transparent;
}
>>> .van-field__label {
  width: auto;
  font-weight: 500;
}
</style>