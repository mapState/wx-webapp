      ...
      <div class="inner">
        <div class="item" v-for="(item,i) in lineOrder" :key="i">
          ...
          <div
            class="tip upDown"
            v-show="i==0"
          >还差{{(item.money/100-item.bonusMoney/100).toFixed(1)}}元够免单</div>
          <div class="right upDown">
            <div class="in upDown" style="width:100%">
              <div class="money">
                <b>{{item.money/100}}</b>元
              </div>待奖励
            </div>
          </div>
        </div>
    ...
    goTo(lat, lon, name, regionAddress, address) {
      wx.ready(() => {
        wx.openLocation({
          ...
          name: regionAddress + address, // 位置名
